(function($){
    var create = function(obj,settings){
        ...
        var sDiv = $('<section class="serviceM-head form-inline"></section>'); // 表格搜索栏
        ...
        if(hDivWidth && isNaN(hDivWidth) && hDivWidth.indexOf('%')>0){
        	hDivWidth=obj.width()*hDivWidth.substring(0,hDivWidth.length-1)/100-2;
        }    
        ...
        if(settings.searchitems){
            ...
            $.each(settings.searchitems,function(k,v){
                ...
                search.append(sp);
                if(v.type){
                    var sch_input = $("<input id='"+ v.name +"' type='"+ v.type+"' name='"+ v.name+"' class='form-control schInput' list='"+ v.list +"' autocomplete='on'>");
                    var schh_input = $("<input type='hidden' name='"+ v.id +"' id='"+ v.id +"' required='true'/>");
                    var sch_list = $("<datalist id='"+ v.list +"'></datalist>");
                    if(v.width){
                        sch_input.css('width',v.width);
                    };
                    search.append(sch_input);
                    search.append(schh_input);
                    search.append(sch_list);
                    if(v.value){
                        sch_input.attr("placeholder",v.value);
                    };
                }else{
            ...
            sDiv.append(form);
            sDiv.append('<button type="submit" id="search" class="search btn btn-default">查询</button>');
            // 此处要给查询和导出添加实现
            $('#search',sDiv).click(function(){
            	var sourceset = $(obj).data('sgDatagrid');
            	var query = {};

            	query = sourceset.query;
                // if(settings.query) query = settings.query;
                var queryArray  =$(".form-group",sDiv).serializeArray();
