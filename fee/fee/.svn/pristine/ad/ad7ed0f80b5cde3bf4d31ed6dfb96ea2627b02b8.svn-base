(function($){
    var create = function(obj,settings){
        ...
        var sDiv2 =$('<section class="serviceM-head form-inline"></section>'); // 表格搜索栏
        ...
        if(settings.searchitems){
            ...
            form.append('<button type="button" class="search btn btn-default">查询</button>');
            if(settings.exporturl){
            	  form.append('<button type="button" class="search btn btn-default">导出</button>');
            };
            sDiv2.append(form);
                // 此处要给查询和导出添加实现
                $('button:first',sDiv2).click(function(){
                	var sourceset = $(obj).data('sgDatagrid');
                	var query = {};

                	query = sourceset.query;
                    // if(settings.query) query = settings.query;
                	 var queryArray  =$("form",sDiv2).serializeArray();
                ...
                // 导出excel
                if($('button',sDiv2).length>1){
                	$('button:last',sDiv2).click(function(){
                		// update by zfy--- begin
                        var queryArray  =$("form",sDiv2).serializeArray();
                        var keyValues=[];
                        var keyArray=[];
      ...
      //搜索栏查询
        $('#search',obj.parent()).click(function(event){
        	var sourceset = $(obj).data('sgDatagrid');
        	var query = {};
