...
public class PaytxtController extends BaseController{
	...
	public void exportPaytxtDt(HttpServletRequest request,HttpServletResponse response) throws SystemException {
		...
		try {
			...
			String[][] title = {{"序号","10"},{"客户名称","14"},{"车牌号码","14"},{"车载电话","14"},{"开通日期","14"},{"费用总额","13","1"},{"服务费","13"},{"服务截止时间","22"},{"SIM卡流量费","13"},{"SIM卡截止时间","22"},{"盗抢险费","13"},{"盗抢险截止时间","22"},{"网查费","13"},{"网查截止时间","22"},{"银行账号","24"},{"银行客户名称","16"},{"托收合同","16"},{"托收合同(文件中)","16"},{"托收机构","12"},{"托收文件名","16"},{"托收标记","16"},{"操作时间","28"}};
			...
			CreateExcel_PDF_CSV.createExcel(valueList, response, "托收明细查询", title,commonCompany.getCnfullname(),commonCompany.getEnfullname(),true);
		} catch (Exception e) {
			LOGGER.error(e.getMessage(), e);
			e.printStackTrace();
		}
