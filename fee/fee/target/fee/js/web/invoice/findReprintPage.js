(function($) {
	...
	var printThis = function() {//打印本页记录
		...
		printType=1;
    	//全选本页数据，再打印
    	if(!$('#frpp-result input[name=checkall]').attr("checked")){
    		$('#frpp-result input[name=checkall]').trigger('click');
    	} 
    	var obj = $('#frpp-result');
		var bDiv = $('.bDiv', obj);
		var rowData=null;
		var checkAll=$('input[type=checkbox][name=frpp-result_chx]', bDiv);
    	...
    	if (checkAll.length == 0) {
			...
			checkAll.each(function() {
				//if ($(this).attr("checked")) {
					...
					reprintIds.push(rowData.reprintId);
		...
		//清除checkall
		if($('#frpp-result input[name=checkall]').attr("checked")){
    		$('#frpp-result input[name=checkall]').removeAttr('checked');
    	} 
		return;
	}
	...
	var deleteChoose = function(){//删除选中记录
		reprintIds=[];
		unitIds=[];
    	printType=3;
    	var obj = $('#frpp-result');
	...
	var printAll = function() {//打印本次选中的记录
    	reprintIds=[];
    	unitIds=[];
    	printType=4;
    	var obj = $('#frpp-result');
    	...
    	$(document).sgConfirm({
			...
			cfn: function(r) {
				if (r) {
		        	...
		        	$("input[name='invoiceType']:checked").each(function() {
		        		var ickv = $("input[name='invoiceType']:checked").val();
		        		var isID = $("input[name='invoiceType']:checked").attr("name");
		        		param[isID] = ickv;
					});
		        	param['customerId']=$('#customerId').val();
		        	param['vehicleId']=$('#vehicleId').val();
		        	param['unitId']=$('#unitId').val();
		        	param['collectionId']=$('#collectionId').val();
		        	param['customerName']=$('#customerName').val();
		        	param['vehicleNum']=$('#vehicleNum').val();
		        	param['unitNum']=$('#unitNum').val();
		        	param['payCtNo']=$('#payCtNo').val();
		        	if(unitIds){
						param['unitIds'] = unitIds.join(',');
					}
		        	
		        	if(reprintIds){
						param['reprintIds'] = reprintIds.join(',');
					}
			    	...
			    	var url='../../invoice/addRePrint';
			    	if((printFun==1 && printType!=3) || printType==5){//打印word
			    		window.location.href='../../invoice/exportAndRePrint?'+keyValues.join('&');
			    	}else if(printType==3){//删除
			    		 $.ajax({
			                 type: "POST",
			                 async:false,
			    	...
			    	//刷新表格
			    	$('#frpp-result').sgDatagrid('reload', 'sgDatagrid');
			    	return true;
				}else{
					return false;
				}
	...
	var loadSuccess = function(){
 		//填充编辑前的值
 		   if(editId && editObj){
 			 	  $("#reprintId",$("#payAccount_add")).val(editObj.reprintId);
 		   	      $("#customerId_form",$("#payAccount_add")).val(editObj.customerId);
				  $("#vehicleId_form",$("#payAccount_add")).val(editObj.vehicleId);
				  $("#unitId_form",$("#payAccount_add")).val(editObj.unitId);
				  $("#customerName_form",$("#payAccount_add")).val(editObj.customerName);
				  $("#vehicleNum_form",$("#payAccount_add")).val(editObj.plateNo);
				  $("#unitNum_form",$("#payAccount_add")).val(editObj.callLetter);
				  $("#printTime",$("#payAccount_add")).val(editObj.printTime);
				  $("#isDel",$("#payAccount_add")).val(editObj.isDel);
 		   }
	...
	var savebill1 = function() {
		//保存
		var params = {};
		var url = '../../invoice/addReprintinvoice';
		
		params.reprintId = $('#reprintId', '#payAccount_add').val();
		if(params.reprintId){
			url = '../../invoice/updateReprint';
		}
		params.customerId = $('#customerId_form', '#payAccount_add').val();
		params.vehicleId = $('#vehicleId_form', '#payAccount_add').val();
		params.unitId = $('#unitId_form', '#payAccount_add').val();
		params.callLetter = $('#unitNum_form', '#payAccount_add').val();
		params.printTime = $('#printTime', '#payAccount_add').val();
		params.isDel = $('#isDel', '#payAccount_add').val();
		if(params.customerId.length<1){
			$(document).sgPup({
				message: 'message_alert',
				text: '请重新输入客户名称并选中！'
			});
			return false;
		}else if(params.vehicleId.length<1){
			$(document).sgPup({
				message: 'message_alert',
				text: '请重新输入车牌号码并选中！'
			});
			return false;
		}else if(params.unitId.length<1){
			$(document).sgPup({
				message: 'message_alert',
				text: '请重新输入车载电话并选中！'
			});
		...
		if(params.printTime){
			params.printTime=params.printTime+"-01";
		}
		$(document).sgConfirm({
			text: '确定保存吗?',
			cfn: function(r) {
	...
	var defaults = {
		...
		$('#frpp-result').sgDatagrid(defaults);
