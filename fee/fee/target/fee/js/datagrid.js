    ...
    var create = function(obj,settings){
        ...
        if(settings.searchitems){
            ...
            $.each(settings.searchitems,function(k,v){
                ...
                search.append(sp);
                if(v.type){
                    var sch_input = $("<input type='"+ v.type+"' name='"+ v.name+"' class='form-control schInput'>");
                    if(v.width){
            ...
            var buttons = $("<div class='pGroup'></div>");
            
            buttons.append('<button type="button" id="search" class="search btn btn-default">查询</button>');
            /*查询实现*/
            ...
            if(settings.exporturl){
            	buttons.append('<button type="button"  class="search btn btn-default">导出Excel</button>');
            	 $('#search ~ button:eq(0)',buttons).click(function(){
            		 var queryArray  =$("form",sDiv).serializeArray();
                     var keyValues=[];
                     var keyArray=[];
                     $.each(queryArray, function(k, v){
